...
<div class="card card-plain">
	<form action="${ctx }/management/projectList/${pj_view.PJ_NO }/modify" method="post" name="fm"> 
		<table class="table table-bordered">
			<tbody>
				<tr>
					<th>프로젝트명 <span style="color: red;">*</span></th>
					<td>
						<input type="text" placeholder="프로젝트명을 입력하세요." name="project_name" class="form-control" maxlength="50" value="${pj_view.PJ_NAME }" />
					</td>
					...
					<td>
						<select id="client_no" class="form-control" name="client_no">
							...
							<c:forEach var="clientList" items="${clientList }">
								<option value="${clientList.CLIENT_NO }" ${pj_view.CLIENT_NO == clientList.CLIENT_NO ? 'selected' : '' } >${clientList.CLIENT_NAME }</option>
							</c:forEach>
						</select>
					</td>
				</tr>
				<tr>
					...
					<td>
						<input type="text" placeholder="프로젝트 시작일을 입력하세요." 
							class="form-control" name="project_start_date" id="project_start_date" value="${pj_view.PJ_START_DATE }" />
					</td>
					<th>프로젝트 종료일 <span style="color: red;">*</span></th>
					<td>
						<input type="text" placeholder="프로젝트 종료일을 입력하세요. "
							class="form-control" name="project_end_date" id="project_end_date" value="${pj_view.PJ_END_DATE }" />
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td colspan="4">
						...
						<div>
							<table id="pj_table" class="table table-hover">
								<thead>
									<tr>
										<th>번호</th>
										<th>사용자</th>
								...
								<tbody id="addUser">
									<c:forEach items="${pj_view.pj_join }" var="project_member">
										<tr>
											<td><input type="text" class="form-control" name="user_no" readonly value="${project_member.MEM_NO }"></td>
											<td><input type="text" class="form-control" name="user_name" readonly value="${project_member.MEM_NAME }"></td>
											<td><input type="text" placeholder="프로젝트 시작일을 입력하세요." class="form-control person_date" name="user_start_date" value="${project_member.MEM_START_DATE }"></td>
											<td><input type="text" placeholder="프로젝트 종료일을 입력하세요." class="form-control person_date" name="user_end_date" value="${project_member.MEM_END_DATE }"></td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
				...
				<tr>
					<th>설명</th>
					<td colspan="4">
						<textarea cols="10" placeholder="내용을 입력하세요." name="project_explanation" class="form-control" style="min-height:130px; resize: vertical;">${pj_view.PJ_EXPLANATION }</textarea>
					</td>
				</tr>
				<tr>
					<td colspan="4">
						<button type="button" class="btn btn-xs btn-default" onclick="location.href='${ctx }/management/projectList'">취소</button>
						<button type="button" class="btn btn-xs btn-default btn-success" onclick="checkVali();">수정</button>
					</td>
				</tr>
			</tbody>
		</table>
...
<script type="text/javascript">
	...
	$(function() {
		$("#project_start_date, #project_end_date, input[name=user_start_date], input[name=user_end_date]").datepicker();
